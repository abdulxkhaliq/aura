...
DEFAULT_CKPT = 'lerobot/smolvla_base'
...
class SmolVLAInferenceNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('checkpoint', DEFAULT_CKPT)
        self.declare_parameter('task', 'Pick the object and place it in the target location.')
        ...
        self.get_logger().info(f'Loading SmolVLA from {ckpt} on {self._dev} ...')
        self._policy = SmolVLAPolicy.from_pretrained(ckpt).to(self._dev)
        self._policy.eval()
        self._policy.reset()

        self.get_logger().info('Building pre/post processors (tokenizer + normalizer) ...')
        ...
        try:
            ...
            self._pre, self._post = make_smolvla_pre_post_processors(
        ...
        self.get_logger().info(
            f'Loaded. Image routing for this checkpoint:')
        for k, src in self._image_src.items():
            self.get_logger().info(f'  {k}  ←  {src}')
        self.get_logger().info(f'Task: {self._task!r}')
        ...
        self.get_logger().info(f'SmolVLA inference node ready, control rate {rate:.1f} Hz.')
    ...
    def _step(self) -> None:
        ...
        batch: dict[str, object] = {'observation.state': state_t, 'task': self._task}
        ...
        if action.shape[0] != NUM_JOINTS:
            self.get_logger().warning(
                f'SmolVLA returned action of shape {action.shape}, expected ({NUM_JOINTS},)',
